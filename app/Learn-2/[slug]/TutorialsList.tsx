...
const TutorialCard: React.FC<TutorialCardProps> = ({ tutorial, index }) => {
  ...
  return (
    <Link href={`${pathname}/${tutorial.slug}`} className="group block">
      <div className="rounded-lg border-2 border-transparent bg-sky-50 p-6 transition-all duration-300 hover:border-blue-600 hover:shadow-lg hover:shadow-blue-600/10">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center space-x-4 text-sm text-slate-500">
                ...
                <div className="flex items-center">
                  ...
                  {`~${Math.ceil(plainTextContent.length / 500)} min read`}
                </div>
              </div>
            </div>
          </div>

          <div className="text-blue-600 transition-colors group-hover:text-blue-800">
            <svg
              className="h-6 w-6"
              fill="none"
              stroke="currentColor"
