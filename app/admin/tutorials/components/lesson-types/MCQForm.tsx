...
const MCQForm: React.FC<MCQFormProps> = ({
  ...
  const [questions, setQuestions] = useState<MCQQuestion[]>(() => {
    if (data && typeof data === "object" && "questions" in data) {
      // New MCQData structure - ensure mermaid_code arrays exist and extract strings
      return ((data as MCQData).questions || []).map((q: any) => ({
        ...q,
                        ...
                        <div key={option.id} className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            <textarea
                              ...
                              rows={2}
                              className="w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:border-slate-600 dark:bg-slate-700 dark:text-white"
                              placeholder={`Option ${String.fromCharCode(65 + optionIndex)} - supports multiline text and code output`}
                            />
                          </div>
